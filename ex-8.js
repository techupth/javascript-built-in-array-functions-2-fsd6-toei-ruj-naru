...
function TotalBills(bill) {
  const newTotal = [];
  let mapLocation = bill.map((item) => item.location);

  mapLocation = mapLocation.filter(
    (item, index) => mapLocation.indexOf(item) === index
  );
  for (let i = 0; i < mapLocation.length; i++) {
    const total = [];
    let filteredBill = bill.filter((item) => {
      if (item.location.includes(mapLocation[i])) {
        return item.total;
      }
    });
    let totalBill = filteredBill.reduce((acc, cur) => {
      return acc + cur.total;
    }, 0);

    total.push([mapLocation[i], totalBill]);
    let obj = Object.fromEntries(total);
    newTotal.push(obj);
  }
